...
[California](http://127.0.0.1:16690/custom/googleVis/GeoChartID3b48744c22a5.html)
...
I suspected that it is because of where the California flights are headed, so I looked to the arrival delays for flights headed from California during the blizzard.
[Kentucky](http://127.0.0.1:16575/custom/googleVis/GeoChartID367445e41b1.html)
```{r plot10, echo = F, message=FALSE, warning=F}
blizzard <- filter(flightdate, FL_DATE == '2016-01-23' | FL_DATE == '2016-01-24'|FL_DATE == '2016-01-22' | FL_DATE == '2016-01-25'|FL_DATE == '2016-01-26')
CAdep <- filter(blizzard, ORIGIN_STATE_ABR == 'CA') %>% group_by(., DEST_STATE_ABR) %>% mutate(., totalarrd = sum(ARR_DELAY_NEW, na.rm=T)/sum(FLIGHTS)) %>% select(., DEST_STATE_ABR, totalarrd) %>% summarise(., arrdelay = max(totalarrd)) %>% arrange(., desc(arrdelay))
...
plot(G3)
```
As it turns out, the highest proportions of arrival delays are concentrated in states that experienced some of the highest snowfall measurements during the blizzard (KY, NJ, AR, MA, PA, GA, IN, MO, OH).               
