...
##The Datasets
The UN Food and Agriculture organization provides information on global Palm oil production and area harvested. The Global Forest Watch has Palm oil mill location data and fire watch data available for review.

##Industry Overview
To navigate to where Palm Oil production is at its highest, we first approach the UN Food and Agriculture:
```{}
...
# Filter for top producers in most recent available year within dataset, year 2014
...
#Calculate percentage of world production
...
#top 5
library(scales)
top.plot<-ggplot(top, aes(AreaName, Production, fill=Percent)) + geom_bar(stat="identity") +
  xlab("Country") + ylab("Oil Production") +
  ggtitle("Palm Oil Production by Country (Top 5)")
firstplot<-top.plot + scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),labels = trans_format("log10", math_format(10^.x)))
firstplot
```
...
The top 5 producers in the world include Indonesia and Malaysia. Indonesia accounts for over 46% or worldwide production. Malaysia accounts for around 34%.

## Industry Map Overview
```{r}
library(maptools)
library(ggplot2)
library(ggmap)
...
plotmapDf <- fortify(plotmap, region = "AreaName") %>% 
  filter(id %in% target)
palmHaMapDf <- merge(plotmapDf, palmHa, 
                     by.x="id", by.y="AreaName")
...
Visually, looking at SouthEast Asia, there are more acres harvested within 
Indonesia. 
...
areaGraph
```
The rise in Area Harvested over time is alarming. The percent change from 2000 to 2014 was % over 186%. (From a value of  2014000
to 740709
...
last.year.fires <-filter(fires, OBJECTID > 11556)
...
map <- leaflet(mills) %>%
  ...
  addCircles(~longitude, ~latitude, popup=mills$type, weight = 3, radius=40, 
...
There is a strong presence of fire hotspots where the majority of Mills are located. 

##Future Questions
Are there any geographic overlaps with existing nature reservations- what is the deforestation risk in those areas? 
How much as the Air Quality Index been impacted?
Which exact species are threatened in each country’s province?
