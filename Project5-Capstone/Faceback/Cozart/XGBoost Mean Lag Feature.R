...
#import the last 4 weeks with 8 week lag features as data.tables
...
#take the mean of the 8 lags

train[, MPriorDemand := (Lag1 + Lag2 + Lag3 + Lag4 + Lag5 + Lag6 + Lag7 + Lag8)/8]
...
for (depth_cv in 1:length(depths)) {
  ...
  write.csv(to_csv, "results_with_lag_feature_priordemandmean.csv", row.names = F)
}
