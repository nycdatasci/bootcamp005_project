...
#make count column for cummulative sum
...
#create running sum of client product pairs

train[, Cum.Sum := cumsum(count), by=list(Cliente_ID,Producto_ID)]

#remove pred

train <- select(train,c(-11,-12))
...
for (depth_cv in 1:length(depths)) {
  ...
  write.csv(to_csv, "results_with_lag_feature_weightpieces_cumsum.csv", row.names = F)
}
