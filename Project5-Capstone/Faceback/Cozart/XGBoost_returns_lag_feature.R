...
week6 <- fread("week6lag8weekreturns.csv")
week7 <- fread("week7lag8weekreturns.csv")
week8 <- fread("week8lag8weekreturns.csv")
week9 <- fread("week9lag8weekreturns.csv")
...
#Add the return lag feature of 3 weeks feature.
...
for (depth_cv in 1:length(depths)) {
  ...
  write.csv(to_csv, "results_with_lag_feature_priordemandtotsum.csv", row.names = F)
}
