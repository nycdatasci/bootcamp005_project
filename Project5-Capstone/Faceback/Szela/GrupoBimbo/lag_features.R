...
train <- fread("train.csv", select = c('Semana', 'Cliente_ID', 'Producto_ID', 'Demanda_uni_equil'),
...
Client_Prod_Week <- train[, mean(Demanda_uni_equil),by = .(Producto_ID, Cliente_ID, Semana)]
Client_Prod_Week$Demanda_uni_equil <- Client_Prod_Week$V1
...
Client_Prod_Week_9$Week9 <- Client_Prod_Week_9$Demanda_uni_equil
Client_Prod_Week_9$Demanda_uni_equil <- NULL
Client_Prod_Week_8 <- Client_Prod_Week[Semana == 8]
Client_Prod_Week_8$Semana <- NULL
Client_Prod_Week_8$Week8 <- Client_Prod_Week_8$Demanda_uni_equil
Client_Prod_Week_8$Demanda_uni_equil <- NULL
Client_Prod_Week_7 <- Client_Prod_Week[Semana == 7]
Client_Prod_Week_7$Semana <- NULL
Client_Prod_Week_7$Week7 <- Client_Prod_Week_7$Demanda_uni_equil
Client_Prod_Week_7$Demanda_uni_equil <- NULL
Client_Prod_Week_6 <- Client_Prod_Week[Semana == 6]
Client_Prod_Week_6$Semana <- NULL
Client_Prod_Week_6$Week6 <- Client_Prod_Week_6$Demanda_uni_equil
Client_Prod_Week_6$Demanda_uni_equil <- NULL
Client_Prod_Week_5 <- Client_Prod_Week[Semana == 5]
Client_Prod_Week_5$Semana <- NULL
Client_Prod_Week_5$Week5 <- Client_Prod_Week_5$Demanda_uni_equil
Client_Prod_Week_5$Demanda_uni_equil <- NULL
Client_Prod_Week_4 <- Client_Prod_Week[Semana == 4]
Client_Prod_Week_4$Semana <- NULL
Client_Prod_Week_4$Week4 <- Client_Prod_Week_4$Demanda_uni_equil
Client_Prod_Week_4$Demanda_uni_equil <- NULL
Client_Prod_Week_3 <- Client_Prod_Week[Semana == 3]
Client_Prod_Week_3$Semana <- NULL
Client_Prod_Week_3$Week3 <- Client_Prod_Week_3$Demanda_uni_equil
Client_Prod_Week_3$Demanda_uni_equil <- NULL

write.csv(Client_Prod_Week, "client_product_mean_per_week.csv", row.names = F)
rm(Client_Prod_Week)
gc()
...
write.csv(week11lag8week, "week11lag8week.csv", row.names = F)
# week11lag8week <- fread("week11lag8week.csv")
# week11lag8week[is.na(week11lag8week)] = 0
for (i in 6:1) {
...
write.csv(week10lag7week, "week10lag8week.csv", row.names = F)
rm(week10lag7week)
gc()
...
write.csv(week9lag6week, "week9lag8week.csv", row.names = F)
rm(week9lag6week)
gc()
...
write.csv(week8lag5week, "week8lag8week.csv", row.names = F)
rm(week8lag5week)
gc()
...
write.csv(week7lag4week, "week7lag8week.csv", row.names = F)
rm(week7lag4week)
gc()
...
write.csv(week6lag3week, "week6lag8week.csv", row.names = F)
rm(week6lag3week)
gc()
...
write.csv(week5lag2week, "week5lag8week.csv", row.names = F)
rm(week5lag2week)
gc()
...
write.csv(week4lag1week, "week4lag8week.csv", row.names = F)
rm(week4lag1week)
gc()
...
week3lag8week$Lag1 <- rep(0, length(week3lag8week$Demanda_uni_equil))
week3lag8week$Lag2 <- rep(0, length(week3lag8week$Demanda_uni_equil))
week3lag8week$Lag3 <- rep(0, length(week3lag8week$Demanda_uni_equil))
week3lag8week$Lag4 <- rep(0, length(week3lag8week$Demanda_uni_equil))
week3lag8week$Lag5 <- rep(0, length(week3lag8week$Demanda_uni_equil))
week3lag8week$Lag6 <- rep(0, length(week3lag8week$Demanda_uni_equil))
week3lag8week$Lag7 <- rep(0, length(week3lag8week$Demanda_uni_equil))
week3lag8week$Lag8 <- rep(0, length(week3lag8week$Demanda_uni_equil))
write.csv(week3lag8week, "week3lag8week.csv", row.names = F)
rm(week3lag8week)
gc()
...
write.csv(week10lag3week, "week_10_with_lag.csv", row.names = F)
write.csv(week11lag3week, "week_11_with_lag.csv", row.names = F)
train_save <- rbind(week9lag3week, week8lag3week, week7lag3week, week6lag3week)
test_save <- rbind(week10lag3week, week11lag3week)
write.csv(train_save, "train_with_lag.csv", row.names = F)
write.csv(test_save, "test_with_lag.csv", row.names = F)
...
submit <- merge(test_save, median_Client_Prod, all.x = TRUE)
submit$M2 <- merge(test_save, median_Prod, by = "Producto_ID", all.x = TRUE)$M2
...
submit$M3 <- NULL
submit$M2 <- NULL
write.csv(submit, "test_with_mde_feature.csv", row.names = F)
